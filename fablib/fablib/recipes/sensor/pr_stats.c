		...
		if (a->bitmap->b_array[i >> 3] & (1 << (i & 0x07))) {
			...
			if ((scc->cpu_user    + scc->cpu_nice + scc->cpu_sys   +
				...
				*scc = *scp;
				...
				printf("cpu.nice %.2f\n", 0.0);
				printf("cpu.sys %.2f\n", 0.0);
				printf("cpu.iowait %.2f\n", 0.0);
				printf("cpu.idle %.2f\n", 0.0);
				printf("cpu.steal %.2f\n", 0.0);
				printf("cpu.hardirq %.2f\n", 0.0);
				 ...
				 unsigned long long itv)
{
	...
	printf("proc.count %.2f\n",
	       S_VALUE   (spp->processes,      spc->processes,      itv));
    
	printf("proc.context_switch %.2f\n",
			       ...
			       unsigned long long itv)
{
    ...
    printf("io.breadps %.2f\n",
            S_VALUE(sip->dk_drive_rblk, sic->dk_drive_rblk, itv));
    printf("io.bwrtnps %.2f\n",
	...
	if (DISPLAY_MEMORY(a->opt_flags)) {
		...
		printf("mem.frmpg %.2f\n",
		       S_VALUE((double) KB_TO_PG(smp->frmkb),
                       (double) KB_TO_PG(smc->frmkb), itv) * 1024);
		printf("mem.bufpg %.2f\n",
		       S_VALUE((double) KB_TO_PG(smp->bufkb),
                       (double) KB_TO_PG(smc->bufkb), itv) * 1024);
		printf("mem.campg %.2f\n",
		       S_VALUE((double) KB_TO_PG(smp->camkb),
                       (double) KB_TO_PG(smc->camkb), itv) * 1024);
	}
	
	if (DISPLAY_MEM_AMT(a->opt_flags)) {
        /* Display instantaneous values */
        printf("mem.free %lu\n", smc->frmkb * 1024);
        printf("mem.used %lu\n", (smc->tlmkb - smc->frmkb)  * 1024);
        printf("mem.buffers %lu\n", smc->bufkb * 1024);
        printf("mem.cached %lu\n", smc->camkb * 1024);
        printf("mem.commit %lu\n", smc->comkb * 1024);
        printf("mem.active %lu\n", smc->activekb * 1024);
        printf("mem.inactive %lu\n", smc->inactkb * 1024);
	}
	
	if (DISPLAY_SWAP(a->opt_flags)) {
        /* Display instantaneous values */
        printf("mem.swapfree %lu\n", smc->frskb * 1024);
        printf("mem.swapused %lu\n", (smc->tlskb - smc->frskb) * 1024);
        printf("mem.swapcad %lu\n", smc->caskb * 1024);
	}
}
				 ...
				 unsigned long long itv)
{
	...
	for (i = 0; i < a->nr; i++) {
		...
		printf("iodev.%s.tps %.2f\n", dev_name, S_VALUE(sdp->nr_ios, sdc->nr_ios,  itv));
		printf("iodev.%s.rd_sec_ps %.2f\n", dev_name, ll_s_value(sdp->rd_sect, sdc->rd_sect, itv));
		printf("iodev.%s.wr_sec_ps %.2f\n", dev_name, ll_s_value(sdp->wr_sect, sdc->wr_sect, itv));
		printf("iodev.%s.avgrq-sz %.2f\n", dev_name, xds.arqsz);
		printf("iodev.%s.avgqu-sz %.2f\n", dev_name, S_VALUE(sdp->rq_ticks, sdc->rq_ticks, itv) / 1000.0);
		printf("iodev.%s.await %.2f\n", dev_name, xds.await);
		printf("iodev.%s.svctm %.2f\n", dev_name, xds.svctm);
		printf("iodev.%s.util %.2f\n", dev_name, xds.util / 10.0);
	}
}
